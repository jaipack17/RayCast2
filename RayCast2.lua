	...
	|- Functions:
		...
		[function] :: RayCast2.new()
			...
			* parameters: (
				...
				[vector2] or [udim2] direction -> Direction point of the ray
			)
...
function RayCast2.new(origin, direction)	
	if not typeof(origin) == "UDim2" and not typeof(origin) == "Vector2" then error("UDim2 or Vector2 expected got "..typeof(origin)) end
	if not typeof(direction) == "UDim2" and not typeof(direction) == "Vector2" then error("UDim2 or Vector2 expected got "..typeof(direction)) end
...
function RayCast2:Cast(parent: Instance, list: table)
	...
	if self._ray then self._ray:Destroy(); self._ray = nil end	
